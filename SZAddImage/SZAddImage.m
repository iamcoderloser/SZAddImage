...
//#define imageH 100 // 图片高度(不能固定)
#define DEFAULTMARGIN 25
#define imageH ((([UIScreen mainScreen].bounds.size.width - DEFAULTMARGIN * 2) - (kMaxColumn - 1) * DEFAULTMARGIN) / kMaxColumn)
//#define imageW 75 // 图片宽度(不能固定)
#define imageW imageH
#define kMaxColumn 5 // 每行显示数量
#define MaxImageCount 10 // 最多显示图片个数
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, 0, width, imageH + 2 * DEFAULTMARGIN);
    }
    return self;
}
...
- (void)changeOld:(UIButton *)btn{
    ...
    if (![self deleClose:btn]) {
        editTag = btn.tag;
        for (NSString *key in btn.layer.animationKeys) {
            if ([key isEqualToString:@"transform.rotation"]) {
                [self stop:btn];
                return;
            }
        }
...
{
    NSInteger row1 = self.images.count / kMaxColumn;
    [self.images removeObject:[(UIButton *)btn.superview imageForState:UIControlStateNormal]];
    NSInteger row2 = self.images.count / kMaxColumn;
    
    if (row2 - row1 < 0) {
        if (_delegate && [_delegate respondsToSelector:@selector(addImageView:isExpand:withHeigth:)]) {
            [_delegate addImageView:self isExpand:NO withHeigth:imageH + DEFAULTMARGIN];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if (editTag == -1) {
        // 创建一个新的控件
        if (self.images.count + 1 >= MaxImageCount) {
            if (_delegate && [_delegate respondsToSelector:@selector(addImageViewOutOfMaxImages:)]) {
                [_delegate addImageViewOutOfMaxImages:self];
            }
            // 退出图片选择控制器
        ...
        if (row2 - row1 > 0) {
            if (_delegate && [_delegate respondsToSelector:@selector(addImageView:isExpand:withHeigth:)]) {
                [_delegate addImageView:self isExpand:YES withHeigth:imageH + DEFAULTMARGIN];
            }
        }
